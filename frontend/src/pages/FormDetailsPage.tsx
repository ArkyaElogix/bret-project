  ...
  if (loading) {
    return (
      <AdminLayout title="Questionnaires Details">
        <p className="text-gray-600 dark:text-gray-200">Loading Questionnaire details...</p>
      </AdminLayout>
    )
  }
  ...
  return (
    <AdminLayout title="Questionnaire Details || View only">
      <div className="max-w-5xl space-y-8">
        ...
        {sections.length === 0 ? (
          <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-6 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200">
            ...
            {sections.map((section) => {
              ...
              return (
                <div key={section.id} className="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                  <div className="border-b border-gray-200 bg-slate-50 px-6 py-4 dark:border-gray-700 dark:bg-gray-900/60">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200">
                          {section.code} - {section.name}
                        </h2>
                        <h2 className="mt-1 text-sm font-bold text-red-800 dark:text-red-400">Instruction displayed to candidates: </h2>
                  ...
                  <div className="divide-y divide-gray-100">
                    {sectionQuestions.length === 0 ? (
                      <p className="p-6 text-sm text-gray-500 dark:text-gray-200">No questions have been added to this section yet.</p>
                    ) : (
                      sectionQuestions.map((q) => (
                        <div key={q.id} className="p-6">
                          <div className="mb-3 flex items-center gap-2">
                            <span className="text-sm font-semibold text-gray-500 dark:text-gray-300">{q.number}.</span>
                            <span className="text-sm text-gray-500 dark:text-gray-300">Question</span>
                          </div>
                          <div className="grid gap-4 md:grid-cols-2">
                            <div className="rounded border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-900/60">
                              ...
                              <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">{q.option_a_text}</p>
                            ...
                            <div className="rounded border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-900/60">
                              ...
                              <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">{q.option_b_text}</p>
